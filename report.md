...
Проект представляет из себя набор файлов с расширением **.ts**. В файле **tsconfig.json** указываются предписание к сборке. Специальной коммандой из множества .ts-файлов компилируется файл build.ts, который уже подключается к HRML-документу в качестве скриптового файла
...
* **Person**. Для него определена система комманд и взаимодействия с ИИ и модулем контроля. Объекты класса **Person** могут передвигаться и выполнять действия. Существуют следующие подтипы **Person**:
