...
Основная часть проекта - игра с двумерной графикой с видом сверху от третьего лица в жанре stealth-action (https://en.wikipedia.org/wiki/Stealth_game). Игроку предоставляется возможность управлять монстром, который может передвигаться, и получать контроль над второстепенными персонажами, переселяясь в них. 
...
* **Перемещение с учётом коллизии со стенами**. Стены представлены как сетка, в каждой клетке сетки стена может либо существовать, либо нет
...
* **Модуль управления**, привязывающийся к любому персонажу. Персонажем является либо Монстр, либо человек
* **Здоровье и получение урона**. У каждого персонажа есть шкала здоровья, которая снижается при получении персонажем урона. Когда здоровье становится не больше нуля, персонаж погибает
* **Стрельба**
  ...
  * **Стрельба из оружия**. Оружием владеют солдаты и могут применять его против игрока, либо же игрок может применять его, взяв контроль над солдатом. Для ограничения стрельбы оружия реализована механика перезарядки и смены обойм
...
* **Анимации**. Реализовано 2 вида анимаций, состоящих из множества кадров:
  * **Анимации персонажей**. Анимация персонажа представляет из себя конечный автомат, где в каждой вершине находится кадр анимации и каждый ход совершается переход к новой вершине в зависимости от состояния персонажа. Например, на анимацию влияет направление движения
  * **"Анимации-призраки"**. Временные анимации, состоящие из зацикленного набора кадров. Определяются начальным и конечным состояниями и временем существования. Состояние включает в себя позицию, размер, угол наклона, прозрачность
* **Камера**. Локации в игре большие, поэтому реализовано перемещение камеры, которая может наводиться на разные участки локации.
* Камера реализована на уровне модуля отрисовки, все объекты отображаются с учётом текущего расположения камеры
* **Освещение**. Реализована механика динамического освещения. На игровой карте расположены источники света, каждый обладающий своей интенсивностью. Динамически рассчитывается освещение для каждой клетки. При отрисовке освещение сглаживается между клетками и выглядит плавным. Источники света могут менять свою интенсивность в процессе игры
...
* **Искусственный интеллект**. Человеческие персонажи управляются искусственным интеллектом. Искусственный интеллект изолирован от прямого взаимодейстивия с характеристиками персонажа (позиция, направление движения). Вместо этого он оперирует командами, которые впоследствии обрабатываются в классе персонажа. ИИ может выполнять следующие действия:
...
Обе части проекта (игра и редактор уровней) используют общие классы для удобства кодирования и расшифровки уровня в json

## Устройство классов
Для приложения используется модель объектно-ориентированного программирования. За каждый процесс отвечает отдельный класс. Далее перечислены основные классы, необходимые для
### Game
...
* **Projectile**. Снаряд, который может двигаться по прямой в пространстве. Особенностью движения являются наличие вязкого трения, ограничивающего скорость, и возможности отскока от стен. Существуют следующие подтипы:
...
* **Body**. Физическое тело. Обладает позицией в пространстве. Для него определён метод `move`, который совершает перемещение тела на заданный вектор в пространстве с учётом коллизии со стенами
...
* **AI**. Класс, отвечающий за искусственный интеллект. Реализует инструкции: 
  * **Ожидание.** Остановка всех действий на указанное время 
  * **Следование к точке** Следование к точке выполняется по сетке коллизий (сетка, наложенная на карту, в каждой вершине которой отмечено существует ли коллизия в данной точке уровня, или нет). Благодаря структурной особенности коллизионной сетки и клеток, можно утверждать, что между любыми двумя соседними клетками существует путь по прямой. Все пути между вершинами коллизионной сетки предподсчитаны в процессе генерации карты при помощи алгоритма **breadth-first search** (https://e-maxx.ru/algo/bfs). Результатом генерации является набор вложенных структур map, которые по двум сериализованным вершинам выдают направление движения. Функция **goToPoint** находит, ближайшие к старту и концу пути, точки коллизионной сетки и строит между ними путь при помощи предподсчитанных значений.
  * **Преследование** Направляет персонажа к точке **lastSeenPoint**, которая обновляется при срабатывании соответствующих триггеров и показывает последнее вероятное местонахождение мимика.
* **BehaviorModel**. Модель поведения. Хранит в себе последовательности инструкций в зависимости от состояния персонажа по настороженности (*normal* либо *panic*). Инструкции интерпретируются в **AI** и переводятся в команды
...
### BehaviorModel
Класс модели поведения объекта. В зависимости от заданной программы поведения и текущего состояния объекта (normal / panic), управляет AI составляющей персонажа, заставляя его действовать согласно предписанной программе.
Класс **BehaviorModel** содержит в себе класс **Instruction**, который в свою очередь является контейнерным классом, содержащим в себе набор команд (см. **AI**) и данных к ним.
...
### PathGenerator
Класс реализующий предподсчёт кратчайших путей. Генерирует коллизионную сетку (сетку наложенную на карту, в каждой вершине которой отмечено существует ли коллизия в данной точке уровня, или нет), а также при помощи алгоритма **breadth-first search** (https://e-maxx.ru/algo/bfs) матрицу предков, ввиде набора вложенных структур map, которые по двум сериализованным вершинам выдают направление движения.

### Draw
Класс, реализующий отрисовку примитивов на HTML-элементе canvas с учётом положения камеры.

Отриcовка примитивов с использованием функционала canvas занимает несколько строчек, что сильно усложняет разработку. Чтобы облегчить использование отрисовки, был создан класс **Draw**, являющийся обёрткой над функционалом canvas, обобщающий отрисовку примитивов
...
Вывод звуков с использованием HTML-элементов и их обработка занимает несколько строчек, что усложняет разработку. Класс создан для облегчения вывода и управления звуками игры.
...
Содержит:
* Класс **Vector**. Двумерный геометрический вектор, содержит параметры x и y. Для вектора реализованы арифметические методы (сложение с вектором, умножение на число), скалярное произведение, получение модуля, определения угла с осью oX
...
* Система контроля версий. Для проекта использовалась система **git**, реализованная через **github**
* Сервис для распределения задач **Trello**. В нём описывадись задачи в порядке приоритета. К каждой задаче крепится список участников, описание и теги, что позволяет существенно облегчить разработку
* **Visual Stdudio Code** для редактирования кода
* Самостоятельно написанный скрипт для разворачивания атласа картинок
* Редакторы **paint** и **gimp** для рисования изображений
* **NodeJS** для сборки

# Личный вклад (мой)
Мной в проект был внесён следующий вклад:
* Идея проекта
* Организация процесса работы внутри команды, выдача задач участникам и контроль их исполнения
* Разработка общей архитектуры приложения
* Класс Game
* Основы класса Draw.ts
* Основа редактора уровней
* Всплывающие анимации
* Снаряды и оружие, механика стрельбы
* Механика переселения
* Класс Vector
* Структура класса Entity и наследников
* Система триггеров
* Модель освещения
* Механика смерти
* Механика экранов загрузки и смерти
* Игровой визуальный интерфейс
* Механика настороженности
